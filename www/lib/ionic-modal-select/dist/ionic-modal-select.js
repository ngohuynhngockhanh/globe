...
/***/ function(module, exports, __webpack_require__) {
	...
	modalSelect.$inject = ["$ionicModal", "$timeout", "$filter", "$parse", "$templateCache", "$rootScope"];
	...
	function modalSelect($ionicModal, $timeout, $filter, $parse, $templateCache, $rootScope) {
		...
		return {
			...
			link: function link(scope, iElement, iAttrs, ngModelController, transclude) {
				...
				scope.ui = {
					...
					resetButton: iAttrs.resetButton || ' ',
					cancelButton: iAttrs.cancelButton || ' ',
				...
				if (iAttrs.optionsExpression) {
					...
					if (!match) {
						throw new Error("ng-repeat expected expression in form of '_item_ in " + "_collection_[ track by _id_]' but got '" + iAttrs.optionsExpression + "'.");
					}
					//var keyExpr = match[1];
				...
				function updateListMode() {
					//shortList controls wether using ng-repeat instead of ng-repeat
				...
				var show = function() {
					console.log(ngModelController.$viewValue, ngModelController.$viewValue)
					if (scope.isChecked && ngModelController.$viewValue) {
						var indexed = {}
						console.log("scope.options", scope.options)
						for (var i = 0; i < scope.options.length; i++) {
							indexed[scope.options[i][1]] = i
						}
						console.log("indexed", indexed)
						for (var i = 0; i < ngModelController.$viewValue.length; i++) {
							var key = ngModelController.$viewValue[i]

							scope.isChecked[indexed[key]] = true
						}
						console.log(scope.isChecked)
						scope.$apply()
					}
					updateTemplate()
					var showModal = scope.modal.show()
					if (shortList) {
						scope.showList = true;
					} else {
						showModal
				...
				$rootScope.$on('open-ionic-modal', function(e, id) {
					if (id == iAttrs.id)
						show()
				})
